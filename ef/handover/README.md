...
Four projects are in-flight right now. Each gets a dedicated page below with everything I'd want to know if I were starting tomorrow: what's done, what's left, what's blocked on whom, and where to look in the code.
...
#### [reth-research · ClickHouse export](#/reth) <span class="pill pill-done">implemented</span>

*durable gas-analysis export*

Optional embedded ClickHouse exporter with a transactional SQLite outbox, deterministic identities, four-table analytical schema, retry/backlog controls, and a staging GitOps migrator. The `gas_analysis` database, scoped account, and initial migration are already in place.
...
| [mattevans/reth compare](https://github.com/mattevans/reth/compare/main...feat/research-clickhouse-export) | embedded `reth-research` ClickHouse export, SQLite outbox, typed row contract, migrations, worker, CLI/config and operator docs |
| [gas-analysis-migrator](https://github.com/ethpandaops/platform/tree/master/environments/staging/applications/gas-analysis-migrator) | staging Argo CD PreSync migrator pinned to reth commit `1f639feed`; initial migration applied |
...
4. **[reth-research](#/reth)** for the independent gas-analysis workstream — exporter architecture, exact ClickHouse row contract, staging migration state, and rollout checks.
5. **[kurtosis](#/kurtosis)** for the end-to-end devnet config I've been using locally — drop-in YAML + run steps.
